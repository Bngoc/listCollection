<?php
/*
 * @Thank to Fabian Schlieper <fabian@fabi.me>
 * @Edit by Pham Dung <phamdung.itcn@gmail.com>
...
class DropboxClient
{
    ...
    const MAX_UPLOAD_CHUNK_SIZE = 314572800;
    ...
    function __construct($app_params, $locale = "en")
    {
        ...
        if (empty($app_params['app_key']))
            throw new DropboxException("Không tìm thấy App Key!");
    ...
    public function GetRequestToken($get_new_token = false)
    {
        ...
        if (empty($rt) || empty($rt['oauth_token']))
            throw new DropboxException('Ko thể lấy token! Kiểm tra lại Key & Secret!');
    ...
    public function UploadFile($type, $src_file, $dropbox_path = '', $overwrite = true, $parent_rev = null)
    {
        if ($type == 'u') {
            ...
            $fop = fopen($src_file, 'r');
            if (!$fop) {
                throw new DropboxException('Liên kết Tập tin  ' . htmlspecialchars(stripslashes($src_file)) . ' không có sẵn. Nó có thể đã bị xóa hoặc bạn nhập một đường dẫn không đúng !');
            } else {
                while ($c = fread($fop, 1024)) {
                    $sss .= $c;
                }
                $file_size = strlen($sss);
            }
        }
        ...
        if ($this->useCurl) {
            ...
            if ($type == 'u') {
                ...
                curl_setopt($context, CURLOPT_INFILESIZE, strlen(file_get_contents($src_file)));
            }
